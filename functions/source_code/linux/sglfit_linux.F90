...
!!$
      ...
      INTEGER ju(nvars)
      INTEGER*4 nbeta(nlam), m(pmax),  gindex(ngroups)
      ...
      Do l = 1, nlam
        ...
        !write(*,'(" sglfitpathF. l, nbeta:",2(i4))')l,nbeta(l)
...
!        write(*,'(" lassofitpathF. l, nbeta:",2(i4))')l,nbeta(l)
...
!!$
   ...
   character(len=80) line
	  ...
	  nlam_pr = mxGetPr(prhs(11))
          ...
          !call mxCopyPtrToInteger4(nlam_pr, nlam, 1)
           call mxCopyPtrToInteger8(nlam_pr, nlam, 1)

           !write(6,'("tmp1:",i6)')tmp1
...
!!$   k=mexPrintf(" FORTRAN"//achar(10))
!!$
!!$   k=mexPrintf(achar(10))
!!$
!!$   write(line,'(" nalam=",i6)')nalam
...
!!$   do l=1,nlam
!!$      !      write(line,*) 'l = ',l
!!$      write(line,'("l, nbeta:",2(i4))')l,nbeta(l)
!!$      k=mexPrintf(line//achar(10)) 
!!$   enddo
...
!!$   
!!$   do l=1,nlam
!!$      write(line,'(i3," alam=",f12.5)')l,alam(l)
!!$      k=mexPrintf(line//achar(10))
!!$   enddo
...
!!$   
!!$   do l=1,pmax
!!$      write(line,'(i6," ibeta=",i6)')l,ibeta(l)
!!$      k=mexPrintf(line//achar(10))
!!$   enddo
!!$
...
!!$
!!$
!!$      write(line,'("tmp1",i4)')tmp1
!!$      k=mexPrintf(line//achar(10)) 


          call mxCopyInteger4ToPtr(nbeta, nbeta_pr, tmp1)
          !call mxCopyInteger8ToPtr(nbeta, nbeta_pr, tmp1)
